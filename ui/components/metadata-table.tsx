      ...
      <table className="w-full text-sm">
        <thead className="bg-muted/50 text-xs uppercase tracking-wider text-muted-foreground">
          <tr>
            ...
            <th className="text-left px-3 py-2">prompt_id</th>
        ...
        <tbody>
          {items.length === 0 && (
            <tr><td colSpan={7} className="px-3 py-6 text-center text-muted-foreground italic">No records.</td></tr>
          )}
          {items.map((m) => (
            <tr key={m.id} className="border-t hover:bg-accent/30 transition-colors">
              ...
              <td className="px-3 py-2 font-mono text-xs text-muted-foreground">{m.prompt_id ?? "-"}</td>
              <td className="px-3 py-2">
                <Badge variant={m.source === "prompt_bank" ? "default" : "secondary"} className="text-[10px]">
                  {m.source === "prompt_bank" ? "prompt_bank" : m.source}
                </Badge>
              </td>
